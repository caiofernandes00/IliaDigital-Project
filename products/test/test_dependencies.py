...
def test_delete_successfully(storage, products):
    storage.delete('LZ127')
    with pytest.raises(storage.NotFound) as exc:
        storage.get('LZ127')
    assert 'Product ID LZ127 does not exist' == exc.value.args[0]
    
def test_delete_inexistent_product(storage, products):
    with pytest.raises(storage.NotFound) as exc:
        storage.delete('invalid_id')
    assert 'Product ID invalid_id does not exist' == exc.value.args[0]
